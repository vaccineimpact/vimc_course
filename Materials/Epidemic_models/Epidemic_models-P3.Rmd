...
if(!dir.exists("docs/Materials/Epidemic_models/Functions")) dir.create("docs/Materials/Epidemic_models/Functions", recursive = TRUE)
...
```{info, transhint, title = "Hint", collapsible = FALSE, include = FALSE}
...
``{info, ref.label = "transhint", title = "Hint", collapsible = FALSE}
``
...
``{r, ref.label = "finalsize", echo = FALSE}
``
...
set.seed(126)
``
``{r, ref.label = "finalsize", echo = -1, eval = FALSE}
``
...
``{r, fig.show = "hold", results = "hold", fig.width = 7, fig.height = 7}
...
``{r, ref.label = "finaltime", echo = FALSE}
``
...
``{r, ref.label = "finaltime", eval = -1, eval = FALSE}
``
