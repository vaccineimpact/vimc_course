...
The aim of this practical is to estimate the age-stratified force-of-infection and basic reproductive ratio from age-stratified serological data.
...
* In section 1 we take the traditional two stage approach of estimating the force-of-infection first and then calculating the next generation matrix from an assumed Who-Aquires-Infection-from-Whom (WAIFW) matrix.  
* In section 2, we take the more elegant approach of estimating the next generation matrix directly using social contact (POLYMOD) data. 
 	 
## Estimating the force-of-infection and $R_0$ of rubella 
 
In this practical we will estimate the force-of-infection of rubella using serological survey data from the UK provided in the book "Modelling Infectious Disease Parameters Based on Serological and Social Contact Data" by Hens *et al.* 

You will need to copy two files from the shared drive that contain the rubella data itself ([`Rubella-UK.dat`](data/Rubella-UK.dat)) and another that defines various helper functions we will use in the practical (`CatalyticModel.R`).
 
We will begin by loading and checking the contents of the rubella data table: 
...
```{r, echo = FALSE}
rubella <- read.csv("data/Rubella-UK.csv", header = TRUE) 
head(rubella)
```

`rubella` should be a table with `r nrow(rubella)` rows and `r ncol(rubella)` columns. `Rubella$Age` is the midpoint of the age group (annual) from which `rubella$Pos + rubella$Neg` samples were taken, of which `rubella$Pos` were positive for Rubella antibodies and `rubella$Neg` were negative. 
 
```{task}
Calculate the probability of being susceptible ($p_s$) within each age category and add this as an extra column to the rubella data frame.
```
...
* You may wish to write a loop, or consider using vectors (e.g. `1:nrow(rubella)`) and `2:(nrow(rubella) - 1)`) to access the different elements of `rubella$p_s`.
``
...
``{r}
rubella$lambda <- NA
rubella$lambda[1:(nrow(rubella) - 1)] <- log(c(1, rubella$p_s[1:(nrow(rubella) - 1)])) - 
    log(rubella$p_s)
``
...
```{r, echo = FALSE, fig.cap = "Proportion susceptible with respect to age"}
## size is a vector to scale the size of points according to the 
## sample size within that age group 
size <- (rubella$Pos + rubella$Neg) / max(rubella$Pos + rubella$Neg) 
...
```{r, echo = FALSE, fig.cap = "First estimate of force-of-infection of Rubella"}
plot(rubella$Age, rubella$lambda, pch = 19, cex = size^0.75)
```
 
```{task, title = "Question"}
Are these "first" estimates of the force-of-infection biologically plausible? If not, how has this transpired and how can we improve the estimates?
```
...
\begin{equation}
    L(K) = \sum_{a} {N_a \choose X_a}p_a(K)^{X_a}\left(1 - p_a(K)\right)^{N_a - X_a}.
\end{equation}

We can code this in R using the following function:

```{r}
## Calculate (Binomial) likelihood of observing given number of positive/negative samples 
## (provided in table sero_data) for each age group and specified by a piecewise continuous 
## force of infection model defined by foi_key
## 
## Arguments:
## x: vector of unique force-of-infection values
## foi_key: pattern vector of length equal to the maximum assumed age in population (or 
## data set) that specifies which unique value of x corresponds to each annual cohort in 
## population.
...
As we did before in Stats Practical 2 we can obtain the maximum-likelihood estimate for this model by optimizing the value of `FOI_loglik()` using the built-in function `optim()`.
...
We can check the fit of the model by plotting the predicted values against the data: 
...
```{solution}
There is a suggestion from Figure \@ref(fig:propsus) that infants, as well as adults, have a lower rate of infection than school age children. Such a "core-group" model will provide a closer fit to the serological age-profile. The log-Likelihood of `r round(threegroup_mle$value, 0)` compared to `r round(twogroup_mle$value, 0)` for the two group model, provides evidence that the three-group model better explains the data even allowing for any penalties we might chose to impose due to the additional parameter (e.g. AIC).
...
We can also estimate the posterior distributions for the two force-of-infections within a Bayesian framework using MCMC. Here we introduce and use one of the standard MCMC libraries in R (`MCMCpack`) that implements a generic MCMC method similar to that used in Stats Practical 2. 
...
Here the chains look well mixed and the algorithm seems to have converged (ideally we would want to run multiple chains). The posterior distributions for the two force of infection parameters are non-overlapping with a risk of infection for school age-children almost twice that of adults.
```
 
## Next-generation matrix approaches
...
We can derive an equation that links the force-of-infection and the elements of the next generation matrix $k_{ij}$. (The details of the derivation are a little messy and will be omitted here, see Anderson & May, Hens et al. for details). 
...
$\mathbf{\psi}$ is a function derived from the force-of-infection that, loosely, corresponds to the average proportion of individuals infectious within each age group at any time. The function `Psi(x, age_widths, TI, u)` defined below returns the vector $\mathbf{\psi}$ for a specified force-of-infection `lambda`, for coarse age groups of widths `age_widths`, and infectious period of length `TI`, and the mortality rate for each age group `u`.
...
```{task}
Use your new expressions to calculate the values of $k$ for the infant mixing and assortative mixing assumptions, and the corresponding values of $R_0$. Use the maximum likelihood estimates of $\lambda_1$ and $\lambda_2$. Assume that the mortality rate $u = 20 / 1000$ and that $T_I = 5 / 365$. Assume the total population size is $n = 1$.

``{info, title = "Hint"}
You will need to use the `max()`, `eigen()` and `matrix()` functions. 
``
```
...
This procedure is straightforward, but a bit fiddly, so once again we have provided you with helper functions below to do this, adapted from the book by Hens et al. 
...
##    in population (or data set) that specifies which unique value of ##    x corresponds to each annual cohort in population.
## sero_data: data frame with columns
## sero_data$Pos: positive samples
## sero_data$Neg: negative samples
    ...
    if(any(x <= 0)) {
        ...
        for(i in 1:nrow(sero_data)) {
            ...
            loglik <- loglik + dbinom(
                sero_data$Pos[i],
                sero_data$Pos[i] + sero_data$Neg[i],
                prob = 1 - sg * exp(-lambda[age_class] * (sero_data$Age[i] - breaks[age_class])),
                log = TRUE
            )
        }
        ## return log-likelihood
...
homogeneous_mle <- optimise(
    ...
    log(c(0.01, 1)),
    maximum = TRUE,
    matrixfunction = HomogeneousMatrix,
    sero_data = rubella
)
homogeneous_mle
```
...
```{r, echo = FALSE}
## load (smoothed symmetrical) POLYMOD matrix 
POLYMOD <- as.matrix(read.csv("data/POLYMOD.csv", header = FALSE))
```
