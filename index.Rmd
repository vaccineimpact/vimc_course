...
> **Note**: the special blocks below cannot be cached, else compiling the document will cause an error. Hence if you want to set a global cache e.g.
> 
> ````
> 
> ```{r}`r ''`
...
> ````
> 
> then you will have to explicitly set `cache = FALSE` in each of the special blocks (e.g. `task`/`solution`) below.
