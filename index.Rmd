...
Sometimes you might want to hide task and/or solution boxes. This can be done with the `renderTask` and `renderSols` chunk options, which can be set globally or locally. For example:
...
```{solution, title = "Answer", renderSols = FALSE}`r ''`
Indeed, though you won't see this answer unless `renderSols = TRUE`...
```
...
```{solution, title = "Answer", renderSols = FALSE}
Indeed, though you won't see this answer unless `renderSols = TRUE`...
```
